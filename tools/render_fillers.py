...
SARVAM_API = "https://api.sarvam.ai/text-to-speech"
CARTESIA_API = "https://api.cartesia.ai/tts/bytes"
CARTESIA_VERSION = "2026-03-01"     # matches pipecat/services/cartesia/tts.py
...
def synthesise_cartesia(text: str, key: str, voice: str, model: str) -> bytes:
    """Render one filler through the LIVE voice.

    The whole design rests on the filler being indistinguishable from the
    sentence behind it -- `fillers.py`: "A filler in a different voice from the
    agent is worse than no filler at all: the caller hears a second person."

    This renderer only spoke Sarvam, and the live TTS moved to Cartesia. So the
    clips could never be keyed to the voice actually on the call, and
    `FillerPlayer._load` found nothing on every call ever placed: built, wired,
    gated, and silent. That is the whole of the "it sounds robotic" report --
    the caller gets dead air where a human says "సరే...".

    Asked for raw PCM at the transport's own rate, so the call path does a disk
    read and nothing else. No decode, no resample, no allocation on the hot
    path.
    """
    body = json.dumps({
        "model_id": model,
    ...
    if not pcm or len(pcm) % 2:
        raise RuntimeError(f"expected raw 16-bit PCM, got {len(pcm)} bytes")
    return pcm
...
def main() -> int:
    ...
    ap.add_argument("--provider", choices=("sarvam", "cartesia"), default="sarvam",
                    help="must match the LIVE tts provider, or the caller hears "
                         "two different people")
    ap.add_argument("--model", default="sonic-3.5", help="cartesia model id")
    ap.add_argument("--list", action="store_true",
                    help="show what would be rendered; makes no API calls")
    a = ap.parse_args()
    # The pipeline lowercases the voice before looking the clips up
    # (run_pipeline.py: `(user_config.tts.voice or "anushka").lower()`), so a
    # Cartesia UUID rendered in any other case is a cache miss and a silent
    # agent -- which is exactly the failure being fixed here.
    a.voice = a.voice.lower()
    ...
    key_name = "CARTESIA_API_KEY" if a.provider == "cartesia" else "SARVAM_API_KEY"
    key = env(key_name)
    if not key:
        print(f"{key_name} is empty in .env -- cannot render.")
        return 1
    ...
    for text in FILLERS:
        ...
        if p.exists():
            ...
            continue
        try:
            pcm = (synthesise_cartesia(text, key, a.voice, a.model)
                   if a.provider == "cartesia"
                   else synthesise(text, key, a.voice))
